...
3. Используемые програмы в проекте:

- `Ansible 2.9.6` ;
- `Openstack 5.2.0` ;
- `Terraform v1.6.5-dev`;
- `Python3` (В Ubuntu 20.04 установлен по умолчанию);
- `Docker 24.0.7` и `Docker-compose version 1.25.0` ([установка](https://docs.docker.com/engine/install/ubuntu/)). Версии в docker образах:
...
4. По умолчанию проект настроен так, что в итоге вы будете наблюдать графики для **трёх** нод. Если вы хотите больше или меньше трёх нод ([подробно](#место-где-нужно-указывать-количество-нод)), то придется корректировать графики в grafana самостоятельно.

5. Обновите репозитории локального сервера, установите `git` и скачайте проект:
```bash
...
```
*Если нужна более гибкая настройка, то следует более подробно ознакомиться с каждой переменной в [официальном проекте LLAMA](https://github.com/dropboxllama).*
```yaml
...
Если вы хотите другую конфигурацию развертываемых серверов:
   ...
   <details>
   <summary>Если вы хотите просмотреть готовые инстант (flavor) или список развернутых серверов</summary>
   Пример вывод команд openstack:
...
```
8. В конце будет вывод ссылки по которой нужно перейти. Пароль от grafana стандартный admin/admin (смените пароль сразу же). В `dashboards` находим llama, переходим по ней и радуемся.
