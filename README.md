...
Убедиться, что вы авторизовались в Openstack API и запустить следующую команду `source rc.sh`.
`terraform init` (Необходимые terraform-providers уже имеются в данном репозитории)
...
После того, как получим положительный результат связанности запускаем плейбук `ansible-playbook /root/Llama/ansible/MEGALLAMA.yml` и ждем...
